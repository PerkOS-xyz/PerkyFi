...
// Mock trade data (will be fetched from API after payment)
const mockTradeData = {
  id: 'trade-001',
...
export default function TradePage() {
  ...
  const copyTxHash = () => {
    navigator.clipboard.writeText(mockTradeData.txHash)
    setCopied(true)
  ...
  const morphoDepositCalls = [
    {
      to: mockTradeData.action.vaultAddress as `0x${string}`,
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-perky-accent/20 bg-perky-accent/10 px-3 py-1 text-sm font-medium text-perky-accent">
            ...
            {mockTradeData.action.type.charAt(0).toUpperCase() + mockTradeData.action.type.slice(1)}
          </span>
          <span className="flex items-center gap-1 text-sm text-gray-400">
            <Clock className="h-4 w-4" />
            {formatDistanceToNow(mockTradeData.timestamp)}
          </span>
        </div>
        <h1 className="text-3xl font-bold">{mockTradeData.prediction.market}</h1>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="card py-4">
          ...
          <div className="text-2xl font-bold text-perky-accent">{mockTradeData.prediction.confidence}%</div>
        </div>
        <div className="card py-4">
          <div className="text-sm text-gray-400 mb-1">Vault</div>
          <div className="text-lg font-semibold">{mockTradeData.action.vault}</div>
        </div>
        <div className="card py-4">
          <div className="text-sm text-gray-400 mb-1">Amount</div>
          <div className="text-2xl font-bold">{formatUSDC(mockTradeData.action.amount)}</div>
        </div>
        <div className="card py-4">
          <div className="text-sm text-gray-400 mb-1">Current APY</div>
          <div className="text-2xl font-bold text-perky-accent">{mockTradeData.action.apy}%</div>
        </div>
      </div>
      ...
      <div className="card mb-8">
        ...
        <p className="text-gray-300 leading-relaxed">{mockTradeData.analysis}</p>
        
        <div className="mt-4 pt-4 border-t border-gray-800">
          <a 
            href={mockTradeData.prediction.sourceUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="text-sm text-perky-primary hover:text-perky-primary/80 flex items-center gap-1"
          >
            View on {mockTradeData.prediction.source}
      ...
      <div className="card mb-8">
        ...
        <div className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg">
          <code className="text-sm text-gray-300 font-mono">
            {mockTradeData.txHash.slice(0, 20)}...{mockTradeData.txHash.slice(-8)}
          </code>
          <div className="flex items-center gap-2">
            ...
            <a
              href={`https://basescan.org/tx/${mockTradeData.txHash}`}
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-lg transition-colors"
            >
